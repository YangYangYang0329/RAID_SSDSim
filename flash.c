				...
				if (update!=NULL)
				{
					...
					if(sub_req->lpn == ssd->stripe.checkLpn){
						if(ssd->trip2Page[sub_req->raidNUM].PChannel == -1){
							...
							/*invaild_page(ssd, ssd->trip2Page[sub_req->raidNUM].location);
							free(ssd->trip2Page[sub_req->raidNUM].location);
							ssd->trip2Page[sub_req->raidNUM].location = NULL;*/
						}
...
struct sub_request * creat_sub_request(struct ssd_info * ssd,unsigned int lpn,int size,unsigned int state,struct request * req,unsigned int operation, unsigned int target_page_type, unsigned int raidID)
{
	...
	**************************************************************************************/
	...
	{    
		...
		if(sub->raidNUM == 0)
			abort();
		/*if(ssd->page2Trip[sub->lpn] != 0){
			if(ssd->page2Trip[sub->lpn] != sub->raidNUM)
				abort();
		}*/
...
void judege(struct ssd_info *ssd, struct sub_request *sub){
	...
	for(chan=0;chan<ssd->parameter->channel_number;chan++)	     
	{
		...
		{   
			...
			if(ssd->channel_head[i].current_state != CHANNEL_IDLE && ssd->channel_head[i].gc_req_nums != -1){
				int add = get_sub_num_channle(ssd, i) - ssd->channel_head[i].gc_req_nums + 1;
				/*if(add < 0){
					printf("%d %d\n", get_sub_num_channle(ssd, i), ssd->channel_head[i].gc_req_nums);
					abort();
				}*/
...
Status 
go_one_step(struct ssd_info * ssd, struct sub_request * sub1,struct sub_request *sub2, unsigned int aim_state,unsigned int command)
{
	...
	****************************************************************************************************/
	if(command==NORMAL)
	{
		...
		{	
			...
			// 	// if (temp_erase_count != 0)
			// 		// printf("tempErase:%llu\n", temp_erase_count);
			...
			// 	// 		// if (temp_erase_count != 0) {printf("tempErase:%llu\n", temp_erase_count); getchar();}
			...
			// 	// //ssd->read_error_rate = 1.163+0.415*((double)(ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].read_count))/1000;
			// 	// //LDPC soft-decision
			// 	// // if (ssd->read_error_rate > 1) {
			// 	// // 	printf("ssd->read_error_rate:%f\n", ssd->read_error_rate);
			// 	// // }
			// 	// // ssd->read_error_rate = 5;
			...
			{   
				...
				***************************************************************************************************************/
				...
				int64_t subBeginTime = sub->begin_time;
				int64_t subEndTime = sub->complete_time;
				int64_t subResponseTime = subEndTime - subBeginTime; //找出sub响应时间
				int chNum = location->channel; //回去找到channel
				 printCHTail(ssd, chNum, subResponseTime);
				/*(sub->readXorFlag == 1){
	...
	else if(command==INTERLEAVE)
	{
		...
		{	
			...
			case SR_R_DATA_TRANSFER:
			{
				...
				ssd->channel_head[location->channel].chip_head[location->chip].current_state=CHANNEL_C_A_TRANSFER;				
				ssd->channel_head[location->channel].chip_head[location->chip].current_time=ssd->current_time;			
				ssd->channel_head[location->channel].chip_head[location->chip].next_state=CHIP_IDLE;			
...
void printCHTail(struct ssd_info *ssd, unsigned int channel, int64_t time) {
	switch (channel) {
		case 0 :
			fprintf(ssd->tailLatencyCh0, "%lld,", time);
			fflush(ssd->tailLatencyCh0);
			break;
		case 1 :
			fprintf(ssd->tailLatencyCh1, "%lld,", time);
			fflush(ssd->tailLatencyCh1);
			break;
		case 2 :
			fprintf(ssd->tailLatencyCh2, "%lld,", time);
			fflush(ssd->tailLatencyCh2);
			break;
		case 3 :
			fprintf(ssd->tailLatencyCh3, "%lld,", time);
			fflush(ssd->tailLatencyCh3);
			break;
		case 4 :
			fprintf(ssd->tailLatencyCh4, "%lld,", time);
			fflush(ssd->tailLatencyCh4);
			break;
		case 5 :
			fprintf(ssd->tailLatencyCh5, "%lld,", time);
			fflush(ssd->tailLatencyCh5);
			break;
		default:
			break;
	}
