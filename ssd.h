...
void getChannelQueueInfo(struct ssd_info *ssd); //得到channel的拥塞程度(即读请求个数和写请求个数)和各个channel是否正在gc
void getSelectedChannel(struct ssd_info *ssd, long long raidID); //得到RMW和RCW选中的channel
void getGcInfo(struct ssd_info *ssd, long long raidID);  //得到pre-read对应的chip是否处在gc状态
void getChannelUpdateTime(struct ssd_info *ssd);
void getChannelDelayTime(struct ssd_info *ssd);
void RMW(struct ssd_info *ssd, struct request *req,long long raidID); //RMW
void RCW(struct ssd_info *ssd, struct request *req,long long raidID); //RCW
