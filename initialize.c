...
*/
	...
	overallTailLatencyInit(ssd);
	updateWriteRatioInit(ssd);
	channelTailLatenInit(ssd);
...
void initAdaptiveChannelInfo(struct ssd_info *ssd) {
	ssd->channelAdaInfo = (struct channelAdaptiveInfo*) malloc (STRIPENUM * sizeof(struct channelAdaptiveInfo));
	alloc_assert(ssd->channelAdaInfo, "ssd->channeAdptivelInfo");
	memset(ssd->channelAdaInfo, 0, STRIPENUM * sizeof(struct channelAdaptiveInfo));
}

void initSelectedChannel(struct ssd_info *ssd) {
	ssd->selChannel = (struct selectedChannel*) malloc (sizeof(struct selectedChannel));
	alloc_assert(ssd->selChannel, "ssd->selChannel");
	memset(ssd->selChannel, 0, sizeof(struct selectedChannel));

	ssd->selChannel->RMWChannelNum = (struct intNums*) malloc (sizeof(struct intNums));
	alloc_assert(ssd->selChannel->RMWChannelNum, "ssd->selChannel->RMWChannelNum");
	memset(ssd->selChannel->RMWChannelNum, 0, sizeof(struct intNums));

	ssd->selChannel->RMWChipNum = (struct intNums*) malloc (sizeof(struct intNums));
	alloc_assert(ssd->selChannel->RMWChipNum, "ssd->selChannel->RMWChipNum");
	memset(ssd->selChannel->RMWChipNum, 0, sizeof(struct intNums));

	ssd->selChannel->RCWChannelNum = (struct intNums*) malloc (sizeof(struct intNums));
	alloc_assert(ssd->selChannel->RCWChannelNum, "ssd->selChannel->RCWChannelNum");
	memset(ssd->selChannel->RCWChannelNum, 0, sizeof(struct intNums));

	ssd->selChannel->RCWChipNum = (struct intNums*) malloc (sizeof(struct intNums));
	alloc_assert(ssd->selChannel->RCWChipNum, "ssd->selChannel->RCWChipNum");
	memset(ssd->selChannel->RCWChipNum, 0, sizeof(struct intNums));
}
...
void initCvInfo(struct ssd_info *ssd) {
	ssd->afterReqChannelInfo = (struct cvInfo*) malloc (STRIPENUM * sizeof(struct cvInfo));
	alloc_assert(ssd->afterReqChannelInfo, "ssd->afterReqChannelInfo");
	memset(ssd->afterReqChannelInfo, 0, STRIPENUM * sizeof(struct cvInfo));
}

void initAdptive(struct ssd_info *ssd) {
	initSelectedChannel(ssd);
	initAdaptiveChannelInfo(ssd);
	initAdaptiveTime(ssd);
	initCvInfo(ssd);
	ssd->reqNum = 0;
}

void overallTailLatencyInit(struct ssd_info *ssd) {
	ssd->overallTailLatency = fopen("tail.csv","w");
}
...
void channelTailLatenInit(struct ssd_info *ssd) {
	ssd->tailLatencyCh0 = fopen("tailCH0.csv","w");
	ssd->tailLatencyCh1 = fopen("tailCH1.csv","w");
	ssd->tailLatencyCh2 = fopen("tailCH2.csv","w");
	ssd->tailLatencyCh3 = fopen("tailCH3.csv","w");
	ssd->tailLatencyCh4 = fopen("tailCH4.csv","w");
	ssd->tailLatencyCh5 = fopen("tailCH5.csv","w");
}

void updateWriteRatioInit(struct ssd_info *ssd) {
	ssd->updateWriteNum = 0;
	ssd->createWriteNum = 0;
}
